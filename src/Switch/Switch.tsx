...
export function Switch(props: SwitchProps) {
  ...
  return (
    <div className={`flex ${alignment === 'top' && 'flex-col'} items-center text-neutral-detail-bolder has-[:disabled]:text-controls-content-disabled`}>
      {label && alignment === 'top' && (
        <label htmlFor={id} className='font-light pb-1 text-sm transition'>
          {label}
        </label>
      )}

      {label && alignment === 'left' && (
        <label htmlFor={id} className='font-light pe-2 text-sm transition'>
          {label}
        </label>
      )}
      ...
      {label && alignment === 'right' && (
        <label htmlFor={id} className='font-light ps-2 text-sm'>
          {label}
        </label>
      )}
    </div>
