...
export function LinkMenuItem({
  ...
  const linkPathname = new URL(link, 'http://placeholder.com').pathname;
  const urlPathname = new URL(url, 'http://placeholder.com').pathname;

  const isSelected = urlPathname === linkPathname;
  ...
  return (
    <>
      <div className='menuItem relative'>
        ...
        <a
          href={enabled ? link : ''}
          className={`${enabled ? 'text-neutral-body' : 'pointer-events-none text-controls-content-disabled'} ${isSelected && 'bg-controls-element-tonal'} link selected:bg-controls-element-tonal flex min-h-5 w-full items-center rounded-md px-3 py-2.5 hover:bg-controls-element-tonal-hover focus-visible:border-controls-highlight`}
          aria-label={label}
          onClick={onNavigate}
        >
      ...
      {isTopLevel && (
        <PopOutContent
          ...
          isContentVisible={isTopLevel && isContentVisible}
          shouldShowSubItems
