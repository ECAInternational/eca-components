...
export function AutoCompleteOptions({
  ...
  return (
    <Transition as={Fragment} leave='transition ease-in duration-100' leaveFrom='opacity-100' leaveTo='opacity-0' afterLeave={() => onClose && onClose()}>
      <Combobox.Options className='absolute mt-2 max-h-60 w-full overflow-auto rounded border border-neutral-detail-paler shadow-lg paragraph-sm-lighter focus:outline-none' {...props} />
    </Transition>
  );
}
